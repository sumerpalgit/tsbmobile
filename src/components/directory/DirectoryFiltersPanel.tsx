 ...
 * against the main window aren't guaranteed to apply there too. This is the same root cause the
 * team already hit and fixed (by dropping `Modal` entirely) for `EventDetailView.tsx`,
 * `MemberProfileView.tsx`, `AdManagement`/`CreateAdCampaign`, `ContributeResource` and
 * `PromptLibrary` — those became pushed screens since they were really full screens; this stays a
 * `Modal` (a genuine slide-in overlay, not a "screen") so it gets its own nested provider instead.
 * The insets-consuming content is split into `DirectoryFiltersPanelContent` because a component
    ...
    <Modal visible={shouldRender} animationType="none" transparent onRequestClose={onClose}>
      <SafeAreaProvider>
        <Animated.View style={[styles.container, { backgroundColor: colors.pageBg, transform: [{ translateX }] }]}>
          <DirectoryFiltersPanelContent visible={visible} filters={filters} sort={sort} stats={stats} onClose={onClose} onApply={onApply} />
        </Animated.View>
      </SafeAreaProvider>
    </Modal>
  );
...
function DirectoryFiltersPanelContent({
  visible,
  filters,
  sort,
  stats,
