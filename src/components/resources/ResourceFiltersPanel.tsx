 ...
 * split into `ResourceFiltersPanelContent` because a component can't read a `Provider` it renders
  ...
  return (
    <Modal visible={shouldRender} animationType="none" transparent onRequestClose={onClose}>
      <Animated.View style={[styles.container, { backgroundColor: colors.pageBg, transform: [{ translateX }] }]}>
        <SafeAreaProvider>
          <ResourceFiltersPanelContent visible={visible} filters={filters} resultCount={resultCount} onClose={onClose} onApply={onApply} />
        </SafeAreaProvider>
      </Animated.View>
    </Modal>
  );
...
function ResourceFiltersPanelContent({
  visible,
  filters,
  resultCount,
  onClose,
