...
export function EventDetailView({
  ...
  if (event.timezone)
    detailRows.push({ label: 'Timezone', value: event.timezone });
  ...
  if (event.visibility)
    detailRows.push({ label: 'Visibility', value: event.visibility });

  const openLink = () => {
    if (!event.event_link) return;
    const url = event.event_link.startsWith('http')
      ? event.event_link
      : `https://${event.event_link}`;
  ...
  return (
    ...
    >
      ...
      >
        ...
        <Text style={[fonts.bold, styles.headerTitle, { color: colors.ink2 }]}>
          Event details
        </Text>
        <Pressable
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        <View style={styles.hero}>
          ...
          <LinearGradient
            colors={['rgba(24,46,67,0.5)', 'rgba(24,46,67,0.86)']}
            style={StyleSheet.absoluteFillObject}
          />
          ...
          <View style={styles.heroContent}>
            <View style={styles.heroBadges}>
              ...
              >
                <Text style={[fonts.bold, styles.typeBadgeText]}>
              ...
              <View
                style={[
                  styles.statusBadge,
                  { backgroundColor: badgeStyle.bg, borderRadius: radius.sm },
                ]}
              >
                <Text
                  style={[
            ...
            <View style={styles.heroDateRow}>
              ...
              <View style={{ minWidth: 0 }}>
                <Text style={[fonts.semibold, styles.heroDateText]}>
                  {formatFullDate(event.start_date)}
                </Text>
                <Text style={[fonts.regular, styles.heroTimeText]}>
        ...
        <View style={styles.body}>
          ...
          >
            ...
            {!!event.location && (
              ...
              >
                ...
                {/* The mockup (`MyEvents.html`) puts a "Map"/"Copy link" button here, but its own
                    handler is named `toastMap` — a toast stub, exactly like the hosted-by button
                    removed above. Real web renders Location as plain text everywhere it appears
                    (`my-events/page.tsx:1226`, `:1289`, `:200`, `:278`) and never opens a map:
                    grepping all of `webSrc` for `maps.google`/`google.com/maps`/`geo:`/`maps.apple`
                    returns nothing, and `my-events` uses no clipboard at all. Removed for parity
                    (confirmed with the user 2026-08-31 — "same as web ditto functionality wise").
                    The one real interaction web DOES have in this block, a clickable event link,
                    mobile already has via `openLink` below. */}
              </View>
            )}
            ...
            >
              ...
              >
                {event.visibility === 'private' ? (
                  <>
                    This event is <Text style={fonts.semibold}>private</Text> —
                    visible only via the registration link.
                  </>
                ) : event.visibility === 'public' ? (
                  <>
                    This event is visible to{' '}
                    <Text style={fonts.semibold}>everyone on TSB</Text>.
                  </>
                ) : (
                  <>
                    This event is visible to{' '}
                    <Text style={fonts.semibold}>{event.visibility}</Text>.
                  </>
                )}
              </Text>
            </View>
          ...
          {!!event.hosted_by && (
            ...
            >
              ...
              {/* The mockup (`MyEvents.html`) puts a "View" button here, but its own handler is
                  literally named `toastHost` — it only ever fires a toast, because there is
                  nothing to open. `hosted_by` is FREE TEXT the event creator types (web validates
                  it as "Please enter who is hosting the event." and posts it as a raw string); it
                  is not a member reference, so no profile can be resolved from it. Real web agrees
                  and renders this block as plain text in all 8 places it appears — including
                  `cards/EventCard.tsx:448` and `mini-cards/EventMiniCard.tsx:223`, which fall back
                  to the POST AUTHOR's name when `hosted_by` is empty, confirming it is a display
                  string. Button removed on that basis (confirmed with the user 2026-08-31) rather
                  than kept as a permanent dead affordance. Do not re-add it from the mockup
                  without a backend field that actually identifies the host. */}
            </View>
          )}
        </View>
      </ScrollView>
      ...
      >
        {isRsvped ? (
          ...
          >
            <Text
              style={[fonts.bold, styles.footerButtonText, { color: '#fff' }]}
            >
              RSVP now
            </Text>
          </Pressable>
        ...
        {!!event.event_link && (
          <Pressable
            ...
            style={({ pressed }) => [
              styles.footerButton,
              { backgroundColor: colors.accentSolid, borderRadius: radius.xl },
              pressed && styles.pressed,
            ]}
          >
